...
class DQN_Solver:
    ...
    def choose_action(self, observation):
        ...
        if self.memory.mem_count > REPLAY_START_SIZE:
            ...
            eps_threshold = 1.0
        # if we rolled a value lower than epsilon sample a random action with a forward bias based on prior knowledge
        if random.random() < eps_threshold:
            return np.random.choice(np.array(range(9)), p=[0.1125, 0.1125, 0.1125, 0.0125, 0.0125, 0.0125, 0.1125, 0.4, 0.1125])
...
for episode in range(EPISODES):
    ...
    while True:
        ...
        if reward >= 40:  # Success condition
            # print("Goal Reached | Episode: ", (episode), " | Reward: ", reward)
            success_count += 1
    ...
    # Save model and display progress
    if (episode + 1) % 100 == 0:
        torch.save(agent.policy_network.state_dict(), "src/Super_Mario_RL_Project/q_table.pth")
        print("Average Total Reward Per Episode (Last 100 Episodes)| Episode: ", (episode + 1), " | Reward: ", episode_batch_score/ float(100))
        print("Average Success Rate (Last 100 Episodes)| Episode: ", (episode + 1), " | Success Rate: ", (success_count),"%")
        episode_batch_score = 0
        success_count = 0
